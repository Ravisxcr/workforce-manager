...
from schemas.attendance import (AttendanceAnalytics, AttendanceCheckIn,
                                AttendanceCheckOut, AttendanceManualEntry,
                                AttendanceOut, AttendanceUpdate)
from services.auth import admin_required, get_current_active_user

router = APIRouter()


def _calc_work_hours(check_in: Optional[datetime], check_out: Optional[datetime]) -> Optional[float]:
...
def get_attendance_analytics(
    ...
    for eid, emp_records in by_employee.items():
        ...
        result.append(AttendanceAnalytics(
            employee_id=eid,
            ...
            avg_work_hours=round(sum(work_hours) / len(work_hours), 2) if work_hours else None,
        ))
    return result
